...
응답헤더 설정하기
	 : 새로운 내용을 DB에 추가했는데도 변경된 내용이 웹브라우저에 
	   출력되지 않는다면 이는 캐시에 저장딘 내용을 출력하고 있기때문이다.
...
파일다운로드 구현시 사용하는 설정
	...
	: 웹브라우저가 인식하지 못하는 MIME타입으로 설정하면
	  웹브라우저는 파일다운도르창을 통해 파일을 다운로드 시켜버린다.
	  jps와 같은 이미지파일은 웹브라우저가 인식하는 MIME타입이므로
	  다운로드를 위해서는 아래와 같은 설정이 필요하다.
*/
//response.setContentType("binary/octect-stream");
/* 
	=> 실행시 주석을 처리하지 않으면 해당 파일이 다운로드 된다.
*/
...
response.setIntHeader("numberOfCrew", 99); // 기존의 헤더값 덮어쓰기(수정)
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<%
	/* 
	getHeaderNames() : 응답헤더의 이름을 얻어온다. 반환타입은 Collection형이다.
	*/
