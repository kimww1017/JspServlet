...
<body>
	...
	<%
		request.removeAttribute("NoKeyValue"); // 키 없으나 에러 발생 X
	...
	<ul>
		...
		새롭게 된달된 페이지의 내용을 출력하게 된다.
		그러므로 사용자는 페이지 이동을 인지하지 못하지만, 내부적으로
		페이지 전달(포워드)이 되는 것이다. 쿼리스트링 형태로 파라미터도 전달할 수 있다.
		...
		요청을 하게 된다. 이 경우 request영역은 공유되지 않는다. 
		웹브라우저의 주소항에는 새롭게 요청된 페이지의 경로명이 보여지게 된다.
		 -->
		<h3>sendRedirect()를 사용한 페이지 이동</h3>
		<%
		response.sendRedirect("RequestResult.jsp?param="+ 
				URLEncoder.encode("리다이렉트방식","UTF_8"));
		%>
</body>
</html>